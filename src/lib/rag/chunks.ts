 ...
 * RAG CHUNKS
 * 
 * These are the text chunks that will be embedded and stored in the vector database.
 * Each chunk is a self-contained piece of information that can be retrieved based on relevance.
 * 
 * Chunking Strategy:
 * - Each chunk is 200-500 tokens (optimal for retrieval)
 * - Chunks are semantically complete (not cut mid-sentence)
...
export interface Chunk {
  ...
  metadata: {
    ...
    subcategory?: string;
    crimeType?: string;
    agent?: 'goody' | 'baddy' | 'both';
    phase?: 'opening' | 'investigation' | 'pressure' | 'resolution';
  };
}
...
You did the right thing today. That matters. Get a good lawyer and don't make me see you in this room again."`,
    metadata: {
      source: "verdict-templates.md",
      category: "verdict",
      subcategory: "confession_cooperative"
    }
  },
  {
...
We got there eventually. Would've been easier for everyone if you'd been straight from the start."`,
    metadata: {
      source: "verdict-templates.md",
      category: "verdict",
      subcategory: "confession_uncooperative"
    }
  },
  {
...
- Gang affiliation`,
    metadata: {
      source: "legal-reference.md",
      category: "legal",
      subcategory: "sentencing_factors"
    }
  },
  {
...
// Export chunk count for reference
export const CHUNK_COUNT = chunks.length;

// Helper to get chunks by category
export function getChunksByCategory(category: string): Chunk[] {
  return chunks.filter(c => c.metadata.category === category);
}

// Helper to get chunks by crime type
export function getChunksByCrime(crimeType: string): Chunk[] {
  return chunks.filter(c => c.metadata.crimeType === crimeType);
}

// Helper to get chunks by agent
export function getChunksByAgent(agent: 'goody' | 'baddy' | 'both'): Chunk[] {
  return chunks.filter(c => c.metadata.agent === agent || c.metadata.agent === 'both');
}
